/*
    ...
    EDIT 4: works for same size inputs; different sizez causes index out of range;
            MUST FIX;
            also mult has a bit of error; trying to solve now      
...
uint16_t mod_digit(uint16_t &x){
    ...
    if((x)<=9){
        ...
        aux=0;// ((x) / (uint16_t)10); //this is /10; but 10=2*5; dont know how to write /5 bitwise
        //x=(x15);
    }
    //cout<<"mod_digit debug else branch: "<<display_cell(x)<<'\n';
    else{
    aux= (uint16_t)(x/10);//-((x>>4)&15) + 1);
    x=(uint16_t)(x%10);
    }
